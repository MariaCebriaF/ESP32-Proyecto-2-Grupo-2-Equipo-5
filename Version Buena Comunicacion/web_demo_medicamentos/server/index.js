...
function typeIdFromName(tipo) {
  const normalized = String(tipo || '').trim().toLowerCase();
  if (normalized === 'paracetamol') return 1;
  if (normalized === 'ibuprofeno') return 2;
  if (normalized === 'enantyum' || normalized === 'amoxicilina') return 3;
  return 0;
}
...
app.post('/api/storage/register', (req, res) => {
  const payload = {
    ...
    tipo_id: typeIdFromName(req.body.tipo),
    tipo: req.body.tipo,
